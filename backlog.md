...
- [ ] **Hide "Backend not configured" banner when offline** — The yellow "Backend not configured" banner takes up prominent space and is irrelevant to most users (the app works fully without backend). Consider hiding it entirely, or replacing with a subtle icon-only indicator. Frees up vertical space especially on mobile.
