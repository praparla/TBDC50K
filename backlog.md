...
- [ ] **Single-source asset version constant** — ISSUE-010 and ISSUE-021 are the same bug class: `sw.js` CORE_ASSETS cache-bust versions drift from `index.html`. Add a `ASSET_VERSION` constant in `config.js` (e.g., `const ASSET_VERSION = 15;`) and reference it in the SW via `importScripts('config.js')` to build CORE_ASSETS paths dynamically. Alternatively, a simple shell script that extracts `?v=` values from `index.html` and patches `sw.js` before deploy. Either approach makes version drift impossible.
