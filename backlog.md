...
- [x] **Consolidate food sections** — Merge "Mandatory Food" and "Food Log" into a single "Food Tracker" section with tabs or sub-headers. Both relate to food tracking at stops. Reduces section count by 1. **DONE** (2026-03-28). Merged Mandatory Food, Food Log, and Calorie Tracker into single "Food & Nutrition" section with 3 tabs (Rules, Log, Calories). Reduces section count by 2.
...
- [x] **Merge race-day TBD sections** — "Split History" and "Segment Records" are both TBD until race day. Consider combining into a single "Race Results" section that shows both split history and segment records. Reduces section count by 1. **DONE** (2026-03-28). Merged Split History, Segment Records, and Finisher Wall into single "Race Results" section with subsections. Auto-hidden when no real data exists.
...
- [x] **Auto-hide empty pre-race sections** — "Finisher Wall", "Split History", and "Segment Records" are empty until race day. Consider hiding them from the sidebar when they have no data, or grouping under a "Race Day" accordion that only appears when race mode is activated. Reduces visible section count by 3 during the planning phase. **DONE** (2026-03-28). Combined into "Race Results" section with `updateRaceResultsVisibility()` — hidden via `display:none` when all data is placeholder/empty.
- [x] **Fix Pace Calculator "Countdown" label mismatch** — The pace note says "Set a start time in the Countdown section" but the actual section is called "Race Day Clock". Update the text to match or make the section name linkable. (UX papercut discovered 2026-03-26) **DONE** (2026-03-27). Changed text to "Race Day Clock section".
- [x] **Merge "Block Parties" into "Party Spots"** — Both are party-related sections. Block Parties shows community-organized parties; Party Spots shows backend-driven parties. Since Block Parties data is also loaded from a JSON file, these could be a single "Parties" section with a "Community" vs "Hosted" distinction. Reduces section count by 1. **DONE** (2026-03-28). Merged into single "Parties" section with Community/Hosted tabs. Reused the scoped tab pattern from Food & Nutrition.
- [x] **Consolidate "Calorie Tracker" with food sections** — Calorie Tracker already references both Mandatory Food and Food Log. Consider merging into the food consolidation (above) so there's one "Food & Nutrition" section with sub-tabs: Mandatory, Log, Calories. Reduces section count by 2. **DONE** (2026-03-28). Included as the "Calories" tab in the consolidated "Food & Nutrition" section.
...
- [x] **Consolidate duplicate `@media (max-width: 768px)` blocks** — `style.css` has 3 separate `@media (max-width: 768px)` blocks (lines ~868, ~2524, ~2883). This caused ISSUE-020 where a later block overrode an earlier one's `.tools-grid` rule. Merge all three into a single block at the end of the file to prevent cascade conflicts and make mobile styles easier to find/audit. **DONE** (2026-03-28). Merged all 3 blocks into the single block at line 927. Only the landscape variant remains as a separate block.
...
- [x] **Single-source asset version constant** — ISSUE-010 and ISSUE-021 are the same bug class: `sw.js` CORE_ASSETS cache-bust versions drift from `index.html`. **DONE** (2026-03-31). Added `ASSET_VERSIONS` object in `config.js` mapping filenames to version numbers. `sw.js` uses `importScripts('/config.js')` and builds CORE_ASSETS dynamically via `versionedPath()`. Test updated to verify `config.js` ↔ `index.html` version sync. Bumped CACHE_NAME to `tb50k-v16`.
- [x] **DRY directions button pattern** — The `openInMaps()` inline onclick button was copy-pasted in 7 popups/panels with identical escaping logic. Extracted into `directionsBtn(lat, lng, label)` helper. Also removed dead `addStravaLinks()` function (duplicate of inline Strava link logic in `buildCourseSectionsList()`). **DONE** (2026-04-01).
...
- [ ] **Event bus for cross-section state** — Sections that share state via localStorage (e.g., Race Day Clock start time → Pace Calculator arrival times, ISSUE-018) must manually call each other's render functions. A lightweight pub/sub bus (`on('startTimeChanged', calculatePace)`) would decouple sections and prevent missed update bugs. Low effort: ~20 lines of JS, replaces ad-hoc `calculatePace()` calls scattered across sections.
