...
- **Service worker cache versioning:** When `index.html` loads assets with cache-bust query strings (`style.css?v=8`), the service worker's `CORE_ASSETS` list must use the same versioned paths. Mismatched paths cause the SW to cache a resource that the page never requests. **Solved:** `ASSET_VERSIONS` in `config.js` is the single source of truth; `sw.js` uses `importScripts('/config.js')` to build CORE_ASSETS dynamically. When bumping a version in `index.html`, update `config.js` `ASSET_VERSIONS` — the test suite catches any drift.
