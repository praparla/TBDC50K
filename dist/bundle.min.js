...
TB_Tools.toggleBlockParties();
TB_Tools.toggleBlockParties();
}
if (TB_Tools.spectatorToggleOn) {
TB_Tools.toggleSpectatorSpots();
TB_Tools.toggleSpectatorSpots();
}
});
});
...
distances.push(cumDist / 1609.34);
}
elevationData = {
distances: distances,
elevations: data.elevation.map(e => e * 3.28084),
...
const { distances, elevations } = elevationData;
const dDist = (distances[j] - distances[i]) * 5280;
if (dDist === 0) return 0;
...
if (absGrade < 2) return '#34d399';
if (absGrade < 4) return '#fbbf24';
if (absGrade < 7) return '#fb923c';
return '#ef4444';
}
...
let factor;
if (grade > 0) {
factor = 1 + 0.035 * grade;
} else if (grade > -5) {
factor = 1 + 0.015 * grade;
} else {
factor = 1 + 0.02 * Math.abs(grade);
}
